...
export default function AdminDashboard() {
  ...
  const handleForceRefresh = async () => {
    try {
      // Call the revalidation endpoint for the homepage
      const response = await fetch(`/api/revalidate?path=/`, {
        cache: 'no-store'
      });
      
      if (response.ok) {
        alert('Homepage refreshed successfully! Try viewing the site in a new tab or incognito window.');
      } else {
        alert('Failed to refresh the homepage. Try restarting the server.');
      }
    } catch (err) {
      console.error('Error refreshing homepage:', err);
      alert('Error refreshing homepage. Please try again or restart the server.');
    }
  };
